...
from i18n_translator import translate_chinese_to_english, translate_filter_chinese_to_english
...
async def search_workflows(
    ...
    """Search and filter workflows with pagination. Supports Chinese input."""
    try:
        # 翻译中文搜索词为英文
        original_query = q
        translated_query = translate_chinese_to_english(q)
        
        # 翻译过滤器值
        trigger_en = translate_filter_chinese_to_english(trigger, "trigger")
        complexity_en = translate_filter_chinese_to_english(complexity, "complexity")
        
        # 如果有翻译，记录日志
        if translated_query != original_query:
            print(f"🌏 中文搜索: '{original_query}' -> 英文: '{translated_query}'")
        
        offset = (page - 1) * per_page
        
        workflows, total = db.search_workflows(
            query=translated_query,  # 使用翻译后的查询
            trigger_filter=trigger_en if trigger_en != "all" else trigger,
            complexity_filter=complexity_en if complexity_en != "all" else complexity,
        ...
        return SearchResponse(
            ...
            per_page=per_page,
            pages=pages,
            query=original_query,  # 返回原始查询以便前端显示
            filters={
                "trigger": trigger,
...
async def translate_query(q: str = Query("", description="Query to translate")):
    """翻译中文查询为英文，提供搜索建议。"""
    try:
        from i18n_translator import translate_chinese_to_english, SERVICE_TRANSLATIONS
        
        translated = translate_chinese_to_english(q)
